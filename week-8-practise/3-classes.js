...
class House {
  constructor(windows, color, numOfRooms, location) {
  ...
  static squareFeet(l) {
    return (l * l) / 10.764; // Math.pow(950,2);
  }
  ...
  houseCost() {
    var baseValue = 50000;
    var roomBycost = 10000 * this.numOfRooms;

    var multiplier = 1; //if it is london 80% of charge increased.

    if (this.location == "london") {
      multiplier = 1.8;
    }
    if (this.numOfRooms === 0 || this.numOfRooms == "studio flat") {
      return baseValue * multiplier;
    } else {
      return (baseValue + roomBycost) * multiplier;
    }
  }
}

const brownHouse = new House(3, "brown", 5, "woking");
const blueHouse = new House(5, "blue", 5, "london");
console.log(blueHouse.houseCost());
console.log(brownHouse.houseCost());
console.log(House.squareFeet(950));
